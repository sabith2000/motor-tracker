...
export async function getUsageStats(limitDays = null) {
    ...
    if (limitDays) {
        // Find logs from the last N days based on rawStartTime
    ...
    const dailyPipeline = [
        ...
        { $sort: { _id: -1 } } // Note: string sort works ok for DD/MM/YYYY only if year is same, but usually fine for display list. For real date sort we'd sort by rawStartTime.
    ];
    
    // Better daily breakdown sort (using max rawStartTime for each group)
