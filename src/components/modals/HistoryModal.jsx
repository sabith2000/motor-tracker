...
export default function HistoryModal({ isOpen, onClose }) {
    // Current filter (null = all time, 7 = last 7 days, 30 = last 30 days)
    ...
    const fetchStats = useCallback(async () => {
        ...
        try {
            const data = await getUsageStats(daysFilter);
            setStats(data);
        } catch (error) {
    ...
    const fetchLogs = useCallback(async (pageNum, isLoadMore = false) => {
        ...
        try {
            // If we have a daysFilter, we still fetch all logs but ideally the API would support range
            // For now, the backend doesn't support generic range filtering except by specific date string
            // We just fetch paginated logs sorted by rawStartTime
    ...
    useEffect(() => {
        if (!isOpen) return;
        fetchStats();
        // Reset logs pagination when filter changes (though right now we just show all logs)
        fetchLogs(1, false);
    }, [isOpen, daysFilter, fetchStats, fetchLogs]);
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-0 sm:p-4">
            <div className="bg-slate-900 w-full h-full sm:h-auto sm:max-h-[90vh] sm:rounded-2xl sm:border border-slate-700 shadow-2xl flex flex-col animate-fade-in-up max-w-4xl overflow-hidden">
                ...
                <div className="flex gap-2 p-4 bg-slate-900 border-b border-slate-800 overflow-x-auto hide-scrollbar flex-shrink-0">
                    {[1, 7, 30, null].map(days => (
                        <button
                            ...
                            className={`filter-chip ${daysFilter === days ? 'filter-chip-active' : ''}`}
                        >
                            {getFilterLabel(days)}
                        </button>
                    ))}
                ...
                <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-6" ref={modalBodyRef}>
                    ...
                    {/* Daily Analytics Mini Chart (Text-based for now) */}
                    {!isStatsLoading && stats?.dailyUsage?.length > 0 && (
                                        ...
                                        <div className="flex-1 mx-3 hidden sm:block">
                                            <div className="h-2 bg-slate-700 rounded-full overflow-hidden">
                                                <div
                                                    ...
                                                    style={{ width: `${Math.min(100, (day.totalMinutes / stats.longestSession) * 100)}%` }}
                                                />
                                            </div>
                                        </div>
